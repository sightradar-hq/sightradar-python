...
class SightRadar:
    ...
    def delete_collection(
        ...
        *,
        ...
        """Schedule an asynchronous cascade delete of a collection.

        By default this is a SOFT delete: the collection stops serving
        immediately but nothing is erased until the grace window elapses, and
        :meth:`restore_collection` undoes it. Pass ``immediate=True`` to start
        erasing now (not restorable), or ``compliance=True`` for an audited
        GDPR/BIPA erasure (implies immediate). Pass ``intent="erase"`` to
        confirm a plain delete of a non-empty collection when the account has
        the destructive-intent guard enforced. Returns 202 with a workflow id;
        poll :meth:`deletion_status` for authoritative progress.
        """
        query: Dict[str, Any] = {}
        if immediate:
    ...
    def delete_photo(
        ...
        *,
        ...
        """Delete the faceprints indexed from one photo (soft by default, restorable)."""
        query: Dict[str, Any] = {}
        if immediate:
            query["immediate"] = "true"
        if compliance:
    ...
    def restore_photo(self, collection_id: str, photo_id: str) -> Dict[str, Any]:
    ...
    def index(
        ...
        *,
        ...
        if file is not None:
            ...
            d = self._request(
                "POST", path, raw_body=body, content_type=ctype,
    ...
    def search_by_id(
        ...
        *,
        ...
        """Search using a previously-stored selfie ``point_id``.

        The engine reads the id from the ``pointId`` field; it must be a UUID or
        an unsigned integer as returned by :meth:`register_selfie` / :meth:`index`.
        """
        payload: Dict[str, Any] = {"pointId": point_id}
        if threshold is not None:
            payload["threshold"] = threshold
        if limit is not None:
            payload["limit"] = limit
    ...
    def register_selfie(
        ...
        *,
        ...
        """Register a single-face selfie you can later search by ``point_id``.

        ``user_id`` is required by the API (it is the Rekognition ``UserId``
        analog). ``selfie_id`` is optional and defaults to a content hash.
        Billable (real-time tier).
        """
        if not user_id:
        ...
        if file is not None:
            ...
            payload = _image_body(url, gcs_key)
            payload["userId"] = user_id
            if selfie_id:
                payload["selfieId"] = selfie_id
            d = self._request(
                "POST", path, json_body=payload, idempotency_key=idempotency_key
            )
        return SelfieResult.from_dict(d)
    ...
    def submit_batch(
        ...
        *,
        ...
        """Submit an async batch job over up to 1,000 URL-only photos.

        Args:
            collection_id: Target collection.
            op: Batch operation — ``"index"`` or ``"match"``.
            photos: List of ``{"url": ..., "external_id": ...}`` descriptors.
                ``external_id`` is optional and is echoed back per photo.
            webhook_endpoint_id: Optional endpoint id from :meth:`register_webhook`.
        """
    ...
    def list_batches(self, *, limit: Optional[int] = None) -> List[Batch]:
        """List batch jobs (most recent first, max 100 per call)."""
    ...
    def get_batch_photos(
        ...
        *,
        ...
        """Per-photo outcomes for a batch, keyset-paginated by ``photo_index``.

        Follow ``next_after_index`` while ``has_more`` is true. This is the
        authoritative pull fallback when a webhook delivery is lost.
        """
        return self._request(
            "GET",
